...
export function step(
  ...
  lineNumber: number,
  threshold: number,
): { state: MachineState; violation: Violation | null } {
  const token: Token | null = tokenizeLine(rawLine, lineNumber);
  if (token === null) {
...
function applyMotion(
  state: MachineState,
  token: Token & { command: MotionCommand },
  lineNumber: number,
  threshold: number,
  rawLine: string,
): { state: MachineState; violation: Violation | null } {
  const get = (letter: "X" | "Y" | "Z" | "E"): number | undefined => {
...
function applyToken(
  stateIn: MachineState,
  token: Token,
  lineNumber: number,
  threshold: number,
  rawLine: string,
): { state: MachineState; violation: Violation | null } {
  let next: MachineState = { ...stateIn };
