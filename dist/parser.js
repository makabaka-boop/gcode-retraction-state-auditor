...
function fail(line, code, message) {
 ...
 */
export function tokenizeLine(raw, line) {
    const text = raw.trim();
    if (text.length === 0) {
        return null;
    }
    ...
    while (cursor < text.length) {
        ...
        if (upper === "G" || upper === "M") {
            ...
            const word = `${upper}${text.slice(cursor + 1, j)}`;
        ...
        if (match === null || match.index !== cursor) {
            ...
            if (peek === undefined ||
                ...
                throw new LineParseError(line, "MISSING_VALUE", `line ${line}: parameter '${upper}' is missing its value`);
            }
            throw new LineParseError(line, "INVALID_NUMBER", `line ${line}: parameter '${upper}' has an invalid numeric value`);
        }
        const value = match[0];
        ...
        if (after !== undefined && after !== " " && after !== "\t") {
            if (!isUpperLetter(after)) {
                fail(line, "INVALID_NUMBER", `parameter '${upper}' has an invalid numeric value starting at '${value}${after}'`);
            }
        }
    ...
    if (command === "G92") {
        for (const key of params.keys()) {
            if (key !== "E") {
                throw new LineParseError(line, "UNEXPECTED_PARAMETER", `line ${line}: G92 only accepts E, found '${key}'`);
            }
        }
        if (!params.has("E")) {
            throw new LineParseError(line, "UNEXPECTED_PARAMETER", `line ${line}: G92 requires an E parameter`);
        }
    }
    else if (command === "G90" || command === "G91" || command === "M82" || command === "M83") {
        if (params.size > 0) {
            throw new LineParseError(line, "UNEXPECTED_PARAMETER", `line ${line}: ${command} takes no parameters`);
        }
    }
    return { command, params };
}
/** Validate the strict decimal grammar. Returns the trimmed canonical text. */
function validateNumber(raw, line, param) {
    if (!NUMBER_RE.test(raw)) {
        throw new LineParseError(line, "INVALID_NUMBER", `line ${line}: ${param} value '${raw}' is not a signed decimal with at most three digits after the dot`);
    }
    if (!Number.isFinite(Number(raw))) {
        ...
        throw new LineParseError(line, "NON_FINITE_NUMBER", `line ${line}: ${param} value '${raw}' is not finite`);
    }
    return raw;
}
/**
 ...
 */
export function parseThousandths(raw, line, param) {
    ...
    if (!Number.isSafeInteger(result)) {
        throw new LineParseError(line, "VALUE_OUT_OF_RANGE", `line ${line}: ${param} value '${raw}' is out of the safe integer range`);
    }
    return result;
}
function isDigit(ch) {
